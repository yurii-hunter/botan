...
type IntegrationTests(app: AppFixture) =
    ...
    let ``should create university`` () =
        task {
            ...
            // Act
            let! response = app.Client.PostAsync("/api/v1/universities", requestBody)
    ...
    let ``should create course`` () =
        task {
            ...
            // Act
            let! response = app.Client.PostAsync("/api/v1/universities/1/courses", requestBody)
            let! responseBody = response.Content.ReadAsStringAsync()
    ...
    let ``should create task`` () =
        task {
            ...
            // Act
            let! response = app.Client.PostAsync("/api/v1/courses/1/tasks", requestBody)
            let! responseBody = response.Content.ReadAsStringAsync()
    ...
    let ``should add solution`` () =
        task {
            ...
            // Act
            let! response = app.Client.PostAsync("/api/v1/tasks/1/solutions", requestBody)
            let! responseBody = response.Content.ReadAsStringAsync()
